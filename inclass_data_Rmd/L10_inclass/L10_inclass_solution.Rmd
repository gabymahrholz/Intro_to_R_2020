---
title: "L10 inclass practice exam"
author: "WT"
date: "09/03/2020"
...
You should end up with a dataframe that has 11280 observations and just three columns. The `L10_inclass_instructions.html` in your downloaded folder will give you some clues about the structure of `ratings_long`.
...
Now look at `words` dataframe. We don't need all these columns. Make a new dataframe called `words_small` that only includes columns `item`, `Word`,`AROUSAL` and `VALENCE`. The `L10_inclass_instructions.html` in your downloaded folder will give you some clues about the structure of `words_small`.
...
Hint: Look at lecture 5 for help with this. The `L10_inclass_instructions.html` in your downloaded folder will give you some clues about the structure of `words_small`.
...
Extra challenge: make the scatterplot points into different colours based on the word type (`condition`). When you are done, your scatterplot should look like the one shown in the `L10_inclass_instructions.html`.
