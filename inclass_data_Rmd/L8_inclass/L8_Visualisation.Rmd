...
traffic_scot <- migration_scot %>% 
  select(DateCode, Value, Sex, `Migration Source`, `Migration Type`, Age) %>% 
...
2. Create a column plot that shows the number of people for each sex using the data we just created using `geom_col()`. Separate the data by using different colours based on `Migration Type`. Don't forget to use `position = 'dodge'` so that your columns do not overlap. 
...
ggplot(boxes, aes(x = `Migration Type`, y = Value)) + 
  geom_violin(aes(fill = `Migration Type`)) + 
  geom_boxplot(aes(alpha = 0.5), show.legend = NULL) +
  ...
  theme(
    ...
    axis.title.y = element_blank(),
    legend.position = NULL, #move the legend to the bottom
    legend.title = NULL
  )
```
...
ggplot(boxes, aes(x = `Migration Type`, y = Value)) + 
  ...
  scale_x_discrete(labels = c('In' = 'Coming to Scotland', 'Out' = 'Leaving Scotland')) +
  theme_bw() +
...
ggplot(traffic_scot2, aes(DateCode, Value,
  ...
  geom_point() + 
  geom_line() +
  scale_x_continuous(limits = NULL, breaks = NULL)
...
ggsave(NULL, NULL, width = 9 , height = 6)
```
